...
// Describes a GLPI asset type: its API endpoints, display label, and optional model field.
export interface AssetTypeConfig {
  itemtype:    string;
  apiType:     string; // API v1 path segment (may differ from itemtype, e.g. "Glpi\\Socket")
  label:       string;
  v2Path:      string; // API v2 path segment
  modelType?:  string;
  modelField?: string;
}
