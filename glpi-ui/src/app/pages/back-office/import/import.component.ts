...
import {AssetImportService} from '@app/core/services/import/asset/asset-import.service';
import {TicketImportService} from '@app/core/services/import/ticket/ticket-import.service';
import {TicketCostImportService} from '@app/core/services/import/cost/ticket-cost-import.service';
import {ImageImportService} from '@app/core/services/glpi/images/image-import.service';
...
export class ImportComponent {
  ...
  readonly stepConfigs: StepConfig[] = [
    ...
    {label: 'Coûts tickets', icon: 'fa-solid fa-coins', accept: '.csv', service: inject(TicketCostImportService)},
    {
  ...
  async startImport(): Promise<void> {
    ...
    for (const {step, i} of validatedSteps) {
      const success = await this.runStep(step.selectedFile!, i);
      if (!success) break;
    }
  }

  private async runStep(file: File, index: number): Promise<boolean> {
